...
configuration_mode = "test"
configuration_checkpoint = "./Model/Gan/"
...
class PredictVgg16:
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            self.saver = tf.train.import_meta_graph('./Model/VGG16/vgg_16_all-ft_model_8000_1.0.meta')
            self.sess=tf.Session()
        with self.sess.as_default():
            with self.graph.as_default():
                self.checkpoint_file=tf.train.latest_checkpoint("./Model/VGG16/")
...
class PredictInceptRes:
    ...
    def predict(self,image_input_resize):
...
def opencv_brightness_from_bytes(image_input):
    hsv = cv2.cvtColor(image_input, cv2.COLOR_BGR2HSV)
    brightness_cv = numpy.average(hsv[:, :, 2])
    return brightness_cv


def opencv_fuzziness_from_bytes(image_input):
    gray = cv2.cvtColor(image_input, cv2.COLOR_BGR2GRAY)
    image_fuzziness = variance_of_laplacian(gray)
    return image_fuzziness
...
def image_save(imagesavepath, image):
    # print("saving image")
    # print(imagesavepath)
    cv2.imwrite(imagesavepath, image, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
def crop_image(image_a):
    ...
    (_, cnts, _) = cv2.findContours(closed.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    return cnts
    # c = sorted(cnts, key=cv2.contourArea, reverse=True)[0]
...
#     # checkpoint_file = tf.train.latest_checkpoint(gd.LATEST_CKPT)
#     # print(gd.LATEST_CKPT)
...
def image_judge(input_array):
    try:
        # create logfile and get the image name by timestamp
        _t = {'all_time': Timer(), 'image_pre_handle_time': Timer(), 'image_brightness_test_time': Timer(),
              'image_fuzziness_test_time': Timer(), 'classification_time': Timer()}
        image_name = str(get_time_stamp())
        log_date = time.strftime("%Y%m%d", time.localtime())
        log_file_writer = open("LOG" + log_date + ".txt", 'a')

        # opencv read file
        image_byte_arry = bytearray(input_array)
        nba = np.asarray(image_byte_arry, dtype=np.uint8)
        image_input_ori = cv2.imdecode(nba, 1)

        # PIL read file
        # image_input_ori = Image.open(BytesIO(input_array))

        _t['all_time'].tic()
        _t['image_pre_handle_time'].tic()
        _t['image_brightness_test_time'].tic()

        #contours = crop_image(image_input_ori)
        label_out = 1
        probability_out = []
        # if len(contours) == 0:
        #     label_out = 0
        #     probability_out = [1.0]
        # else:
        image_input_resize = cv2.resize(image_input_ori, (gd.INPUT_SIZE, gd.INPUT_SIZE))

        probability_out,label_out=InceptRes_predict.predict(image_input_resize)

        if label_out[0][0]==0:
            origin = cv2.resize(image_input_resize, (512, 512))
        ...
        #probability_out, label_out = sess.run([probability, label], feed_dict={image: image_input_resize})
        okthreshold = 0
        ngthreshold = 0
        # print("label:%d, prediction:%d, confidence score:%f" %(0,real_predict,real_confidence_score))
        print(probability_out)
        print(label_out)
        print(probability_out[0])
        print(label_out[0])
        print(probability_out[0][0])
        print(label_out[0][0])

        print(type(probability_out[0][0]))
        print(type(label_out[0][0]))
        if ((label_out[0][0] == 0) and (probability_out[0][0] > okthreshold)):
            test_result_buffer = struct.pack("B", gd.OK_FLAG)
            test_result_buffer += struct.pack("B", len(probability_out[0]))
            print("OK_FLAG")
            for i in range(0, len(probability_out[0])):
                test_result_buffer += struct.pack("I", 0)
                test_result_buffer += struct.pack("I", 0)
                test_result_buffer += struct.pack("I", 0)
                test_result_buffer += struct.pack("I", 0)
                test_result_buffer += struct.pack("I", label_out[0][i])
                test_result_buffer += struct.pack("I", int(probability_out[0][i] * 1000))
                print("prediction:%d, confidence score:%f" % (label_out[0][i], probability_out[0][i]))

        elif ((label_out[0][0] != 0) and (probability_out[0][0] > ngthreshold)):
            test_result_buffer = struct.pack("B", gd.NG_FLAG)
            test_result_buffer += struct.pack("B", len(probability_out[0]))
            print("NG_FLAG")
            for i in range(0, len(probability_out[0])):
                test_result_buffer += struct.pack("I", 0)
            ...
            test_result_buffer = struct.pack("B", gd.UNSURE_FLAG)
            test_result_buffer += struct.pack("B", len(probability_out[0]))
            print("UNSURE_FLAG")
            for i in range(0, len(probability_out[0])):
                test_result_buffer += struct.pack("I", 0)
        ...
        _t['classification_time'].toc()
        log_file_writer.writelines((" class$" + str(label_out[0][0]) + "$probability$" + str(
            int(probability_out[0][0] * 1000)) + "$classification_time:{:.5f}$").format(
            _t['classification_time'].average_time))
        _t['all_time'].toc()
        log_file_writer.writelines(("all_time:{:.5f}$\n").format(_t['all_time'].diff))

        return test_result_buffer

    except Exception as e:
        print(str(e))
        exception_file_writer = open("EXCEPTION_" + log_date + ".txt", 'a')
        exception_file_writer.write(str(image_name) + '|' + '\n')
        exception_file_writer.write(str(e))
        test_result_buffer = struct.pack("B", gd.EXCEPTION_ERROR_FLAG)
        test_result_buffer += struct.pack("B", 0)
        return test_result_buffer
...
image = tf.placeholder(tf.float32, shape=[gd.INPUT_SIZE, gd.INPUT_SIZE, 3])

image_reshape = tf.reshape(image, [-1, gd.INPUT_SIZE, gd.INPUT_SIZE, 3])
print(image_reshape)
with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()) as scope:
    logits, description = inception_resnet_v2.inception_resnet_v2(image_reshape, num_classes=gd.NUM_CLASS,
                                                                  is_training=False)
print(description)
...
# label_out = tf.argmax(logits, axis=1)
probability, label = tf.nn.top_k(softmax, k=5)
sess = tf.Session()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        src_dir = gd.DIR_FOR_TEST

    csvfile = open(gd.DIR_FOR_RESULT + "/test_detail_abe_interface.csv", "w")
    ...
    for dirc in os.listdir(src_dir):
        ...
        print(filename)
        fd = open(filename, 'rb')
        print(image_judge(fd.read()))
        # class_o,probability_o=image_judge(filename)
        # print(class_o)
        # print(probability_o)
